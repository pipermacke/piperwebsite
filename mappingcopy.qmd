...
# Creating Chloropleth Maps in R
...
forestdata <- read_csv('forestarea.csv',  problems())
...
First, I want to find the true average of U.S. incomes in the dataset in order to have a meaningful midpoint within the divergent scale, so that can be found by doing the following:
...
I actually find that mapping the median income of U.S. counties on a diverging scale is more meaningful when it comes to seeing which counties are below the poverty line and which are way above it. That beind said, because I was using just the median income of this dataset and not a value that draws the line for what income is considered in poverty and what is not, it's not necessarily correct to conclude that those below the midpoint in the diverging scale are in poverty.
